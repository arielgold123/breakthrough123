...
def main ():
    ...
    best_res = -200
    loss_count = 0
    tester1 = Tester(player1=player1, player2=Randomagent(player=2, env=env), env=env)
    tester2 = Tester(player1=Randomagent(player=1, env=env), player2=player2, env=env)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        while not env.is_end_of_game(state_1):
            ...
            next_actions = next_actions.reshape(-1,4)
            with torch.no_grad():
            ...
            if loss_count <= 1000:
                ...
                avgLoss1 += (loss.item()-avgLoss1)* 0.00001 
        ...
        if (epoch+1) % 100 == 0:
            ...
            if best_random < test_score1 :
                best_random = test_score1
                player1.save_param(path_best_random1)
            print(test1)
            random_results.append(test_score1)
            
            test2 = tester2(100)
            test_score2 = test2[0]-test1[1]
            if best_random < test_score2 :
                best_random = test_score2
                player1.save_param(path_best_random2)
            print(test2)
            random_results.append(test_score2)
        ...
        print (f'learning rate={scheduler1.get_last_lr()[0]} path={path_Save} res= {res} best_res = {best_res}')
